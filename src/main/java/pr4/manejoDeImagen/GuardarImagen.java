...
public class GuardarImagen implements IManejoDeImagen{
    ...
    public void hacer(JFrame parent, Imagen modelo) throws ManejoDeImagenException {
        ...
        if(modelo.getHeight() <=0 | modelo.getWidth() <= 0){
            throw new ManejoDeImagenException(ManejoDeImagenException.IMAGEN_NULA_MESSAGE);
        }
        ...
        try {
            for (int x = 0; x < modelo.getWidth(); x++) {
                for (int y = 0; y < modelo.getHeight(); y++) {
                    ...
                    imagen.setRGB(x, y, modelo.getPixeles()[y][x]);
                }
            }
            ...
            if(nombreImagen.isEmpty()){throw new ManejoDeImagenException(ManejoDeImagenException.NOMBRE_VACIO_IMAGEN_MESSAGE);}
            if(!nombreImagen.matches("[a-zA-Z0-9 ]+")){throw new ManejoDeImagenException(ManejoDeImagenException.NOMBRE_INVALIDO_IMAGEN_MESSAGE);}
